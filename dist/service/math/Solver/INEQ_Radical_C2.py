...
def step_c2(function, varList):
    ...
    left_expr = (simplify_function.lhs)**2
    right_expr = simplify_function.rhs

    first_step = []
    first_step_a = []
    first_step_a.append(ineq_json(left_expr, " \geq ", 0))
    first_step_a.append(ineq_json(right_expr, " \leq ", 0))
    first_step_b = []
    first_step_b.append(ineq_json(right_expr, " \geq ", 0))
    first_step_b.append(ineq_json(left_expr, " \geq ", right_expr**2))
    first_step.append(and_json(first_step_a))
    first_step.append(and_json(first_step_b))
    step.append(longor_json(first_step))


    second_step = []
    second_step_a = []
    second_step_a.append(solveSimply_step(left_expr, ">=", 0, varList))
    second_step_a.append(solveSimply_step(right_expr, "<=", 0, varList))
    second_step_b = []
    second_step_b.append(solveSimply_step(right_expr, ">=", 0, varList))
    second_step_b.append(solveSimply_step(left_expr, ">=", right_expr**2, varList))
    second_step.append(and_json(second_step_a))
    second_step.append(and_json(second_step_b))
    step.append(longor_json(second_step))

    step.append(solveSimply_last_step(function, varList))

    return step


def solve_radical_ineqSBS_c2(function, varList):
    datareturn = {}
    ...
    if len(varList) > 0:
        ...
        datareturn["step"] = step_c2(function, varList)
        datareturn["classification"] = "ineq_c2"
