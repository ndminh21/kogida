...
def ineq_json_poly(function1, function2, op, val):
    if op == ">=":
        op = " \geq "
    elif op == "<=":
        op = " \leq "
        ...
        op = " < "
    data = {}
    data["val"] = latex(function1 - function2) + " " + latex(function1 + function2) + op + latex(val)
    res_data = {}
    res_data["eq"] = data
    return res_data
...
def step_abs_ineq_t8(function, varList, classification):
    step = []
    ...
    step.append(ineq_json_original(left.args[0]**2, op, right.args[0]**2))

    step.append(ineq_json_original((left.args[0]-right.args[0])*(left.args[0]+right.args[0]), op, 0))

    # step.append(ineq_json_poly(left.args[0], right.args[0], op, 0))

    step.append(solveSimply_step(left.args[0]**2, op, right.args[0]**2, varList))

    return step

def ineq_t8(function, varList, classification):
    start_time = time.time()
    data = {}
    data["step"] = step_abs_ineq_t8(function, varList, classification)
    data["root"] = solveSimply(function, varList)
    data["time"] = time.time() - start_time
    data["classification"] = "ineq_t8"
    data["category"] = "ineq"
