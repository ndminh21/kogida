...
def solve_eq_viet(function, var, arr_var, nagative):
    ...
    if len(result) == 0:
        ...
        for item in result:
            ...
            datareturn["eq_viet"] = end_viet_longor(and_json(childs1))
        ...
        if nagative == 1:
            ...
            step.append(end_viet_longor([and_json(childs1), and_json(childs2)]))
            ...
            childs2.append(eq_json_lhs_rhs(simplify(arr_var[0]), -childs[1]))
            childs2.append(eq_json_lhs_rhs(simplify(arr_var[1]), childs[0]))
            step.append(longor_json([and_json(childs1), and_json(childs2)]))
            
            datareturn["eq_viet"] = step
...
def eqsys_viet2(arr_eq, arr_var, classification):
    ...
    if classification["nagative"] == 1:
        step.append(var_viet_json(["-x", "y"]))
    else:
        step.append(var_viet_json(["x", "-y"]))

    step.append(start_viet_json(new_func))
    step.append(solve_eq_step(new_func, ['X']))
    step_root = solve_eq_viet(new_func, ['X'], arr_var, classification["nagative"])
    for item in step_root["eq_viet"]:
        step.append(item)
    data = {}
    data["step"] = step
    data["root"] = step_root["root"]
    data["time"] = time.time() - start_time
    data["classification"] = "eqsys_viet2"
    data["category"] = "eqsys"
