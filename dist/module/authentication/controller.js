...
class AuthenticationController {
    static route(router) {
        ...
        router.get('/auth/google/callback', passport.authenticate('google', { failureRedirect: '/dang-nhap', failureFlash: true, failureMessage: "Tài khoản đã bị cấm sử dụng" }), function (req, res) {
        ...
        router.get('/auth/facebook/callback', passport.authenticate('facebook', { failureRedirect: '/dang-nhap', failureFlash: true }), function (req, res) {
            let redirect = req.flash('redirectUrl');
            let method = req.flash('method');
            if (redirect.length != 0 && method.length != 0 && method[0] == 'get')
                res.redirect(redirect[0]);
        ...
        router.post('/dang-nhap', passport.authenticate('local', { failureRedirect: '/dang-nhap', failureFlash: true }), function (req, res) {
            let redirect = req.flash('redirectUrl');
            let method = req.flash('method');
            if (redirect.length != 0 && method.length != 0 && method[0] == 'get')
                res.redirect(redirect[0]);
