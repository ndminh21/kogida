...
def step_c8(classification, varList):
    step = []
    ...
    right_expr = (simplify_function.rhs)**2

    first_step = []
    first_step.append(ineq_json(left_expr, " \geq ", 0))
    first_step.append(ineq_json(left_expr, "\geq ", right_expr))
    step.append(and_json(first_step))

    second_step = []
    second_step.append(solveSimply_step(left_expr, ">=", 0, varList))
    second_step.append(solveSimply_step(left_expr, ">=", right_expr, varList))
...
def solve_radical_ineqSBS_c8(classification, varList):
    datareturn = {}
    ...
    if len(varList) > 0:
        ...
        datareturn["step"] = step_c8(classification, varList)
        datareturn["classification"] = "ineq_c8"
